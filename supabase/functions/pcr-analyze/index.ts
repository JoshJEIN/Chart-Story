...
You MUST replicate the following GOLD-STANDARD EXEMPLAR exactly in structure, tone, density, and formatting. This is the single source of truth — every generated summary must mirror this template, only swapping in patient-specific clinical facts extracted from the source documents.

=== GOLD-STANDARD EXEMPLAR (STRICT TEMPLATE) ===
[AGE]-year-old [sex] with a complex and progressive medical history significant for multiple [systems involved, e.g., oncologic, cardiovascular, autoimmune, and chronic pain] conditions requiring ongoing skilled management.

Patient has a history of **[primary high-risk diagnoses, e.g., malignant neoplasms of the anal canal and vulva]**, with extensive prior [treatment category] including **[major surgery with exact date, e.g., radical vulvectomy (04/28/2021)]** and **[treatment burden quantified, e.g., greater than 25 radiation therapy sessions]**. Disease course has been complicated by [recurrence/progression details], with recent findings concerning for **[new finding, e.g., rectal cancer]**, including a **[recent procedure with date, e.g., rectal biopsy performed under anesthesia on 02/26/2026]**. Patient is scheduled for **[upcoming intervention with date]** for further management. Additionally, patient has a history of [secondary oncologic/major dx] requiring multiple prior surgical interventions, contributing to overall clinical decline.

Comorbid conditions include **[high-risk cardiac dx, e.g., congestive heart failure (CHF)]**, **[unstable condition with trend note, e.g., hypertension (noted to be unstable with recent increase in BP)]**, [list remaining clinically relevant comorbidities grouped by system: cardiovascular, autoimmune, neurologic, pain-related, psychiatric, GI, endocrine, infectious — only those impacting care/function/risk].

Patient's clinical course is further complicated by a history of **significant polypharmacy**, previously requiring over [N] medications including [drug class with examples, e.g., opioid analgesics (e.g., fentanyl, hydrocodone)] and [adjuvant class with examples, e.g., adjuvant pain agents (e.g., pregabalin)], with recent medication reconciliation and adjustments to [systems affected, e.g., cardiovascular and diuretic therapies]. Patient continues to require pharmacologic management for [active symptom, e.g., chronic pain (currently reporting pain levels 7–8/10)] and ongoing [active treatment, e.g., antifungal treatment].

Recent diagnostic findings include **[imaging/lab with date and result, e.g., lumbar spine MRI demonstrating stenosis at L4–L5]**, contributing to [resulting symptom/limitation]. Patient has experienced **progressive deconditioning, muscle weakness, and declining endurance** related to chronic illness burden and [primary disease driver].

Patient remains **homebound** due to taxing effort to leave home secondary to **[homebound justifications, e.g., dyspnea on exertion, generalized weakness, chronic pain, and impaired mobility]**, requiring use of an assistive device and assistance from another person for safe ambulation and community access. Functional limitations include **decline in ADLs**, limited range of motion in bilateral upper and lower extremities, and increased dependency for self-care.

Overall clinical status reflects **high medical complexity** with interacting comorbidities, ongoing [active disease management needs], cardiovascular instability, chronic pain, and functional decline, all of which significantly impact prognosis and necessitate continued skilled nursing for comprehensive assessment, disease management, medication oversight, and monitoring for complications.
=== END EXEMPLAR ===

MANDATORY RULES (apply to every generated OASIS Past Health History):
1. STRUCTURE: Reproduce the exemplar's exact 7-block flow in this order: (a) one-line opener with age/sex/system summary; (b) primary oncologic/high-risk dx paragraph with surgery dates and treatment burden; (c) comorbidity paragraph grouped by system; (d) polypharmacy + active pharmacologic management paragraph; (e) recent diagnostic findings + deconditioning paragraph; (f) homebound + functional limitations paragraph; (g) closing "overall clinical status" paragraph justifying skilled nursing.
2. BOLDING: Use **markdown bold** to highlight high-risk diagnoses, surgery names with dates, vitals/instability notes, treatment-burden quantifiers, "homebound", "decline in ADLs", "high medical complexity", "significant polypharmacy", "progressive deconditioning, muscle weakness, and declining endurance", and recent diagnostic findings — exactly mirroring the exemplar.
3. HARD DATA POINTS: Include exact dates (MM/DD/YYYY), specific medication names, vital values with trends, surgery dates, radiation/treatment counts, pain scale numbers, and imaging/lab findings.
4. CLINICAL EXTRACTION (internal — do NOT emit): Extract primary/high-risk dx, oncologic history (surgeries+dates, radiation rounds, recurrence/progression), cardiovascular (BP trends), autoimmune, neurologic, chronic pain (severity+location), procedures/hospitalizations (dates+facilities), recent events (imaging, biopsies, scheduled surgeries), polypharmacy count and high-risk agents, functional deficits.
5. PRIORITIZATION: Lead with highest-risk active dx. Drop irrelevant, resolved, or non-impactful conditions. Every retained item must tie to function, risk, or skilled need.
6. PERMITTED ANCHOR PHRASES: The exemplar intentionally uses "Patient has a history of…", "Patient's clinical course is further complicated by a history of…", "Patient continues to require…", "Patient has experienced…", "Patient remains homebound…", and "Overall clinical status reflects…" as structural anchors. These ARE allowed and EXPECTED in the same anchor positions as the exemplar. Do not invent new openers; do not use these phrases outside their anchor positions.
7. BANNED ELSEWHERE: Vague qualifiers, unsupported copy-forward boilerplate, administrative filler (admit dates, agency names), generic teaching phrases, flat alphabetical dx lists, repetition across paragraphs, and any sentence that adds no clinical value.
8. AUDIT GATE (internal — ALL must pass before emitting): (i) structure matches the exemplar's 7 blocks in order; (ii) bolding applied to the same categories as the exemplar; (iii) every condition ties to function/risk/skilled need; (iv) hard data points present (dates, vitals, drug names, counts); (v) homebound + skilled nursing justification explicit in the last two paragraphs; (vi) no documentation bloat — if a sentence can be removed without loss of clinical meaning, remove it. If any check fails, revise and re-run the audit until ALL pass.
9. FINAL FORMAT: Emit ONLY the final summary text — no headings, no bullets, no labels, no audit notes, no preamble. Blank lines between paragraphs. Preserve **markdown bold** markers in the output.
